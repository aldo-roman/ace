...
export const Generator = function(source) {
...
export const makeAsync = function(args, fn, context) {
    if (fn.length > args) 
        return fn
    else {
        return function() {
...
export const list = function(arr, construct) {
    var construct = construct || Generator
    var i = 0
    var len = arr.length
...
export const values = function(map, construct) {
    var values = []
    for (var key in map) 
        values.push(map[key])
        
    return list(values, construct)
}

export const keys = function(map, construct) {
    var keys = []
    for (var key in map) 
        keys.push(key)
        
    return list(keys, construct)
}
 ...
 */ 
export const range = function(start, stop, step, construct) {
    var construct = construct || Generator
    start = start || 0
    step = step || 1
...
export const concat = function(first, varargs) {
...
export const zip = function(first, varargs) {
    if (arguments.length > 1)
...
export const plugin = function(members, constructors) {
    if (members) {
        for (var key in members) {
            Generator.prototype[key] = members[key]
        }
    }
